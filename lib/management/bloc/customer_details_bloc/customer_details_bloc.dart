...
class CustomerDetailsBloc extends Bloc<CustomerDetailsEvent, CustomerDetailsState> {
  ...
  final CustomerManagementService _customerManagementService = Get.find<CustomerManagementService>();
  final AuthenticationService _authenticationService = Get.find<AuthenticationService>();
  CustomerDetailsBloc() : super(CustomerDetailsState(type: CustomerDetailsStateType.loading, customerInfo: null));
  ...
  Stream<CustomerDetailsState> mapEventToState(
    ...
    if (event is FetchCustomerDetails) {
      CustomerDetailsState(type: CustomerDetailsStateType.loading, customerInfo: null, accountInfo: null);
      ...
      yield CustomerDetailsState(
          type: CustomerDetailsStateType.fetched, customerInfo: customerInfo, accountInfo: accountInfoResponse);
    } else if (event is LogOut) {
      CustomerDetailsState(
          type: CustomerDetailsStateType.loading, customerInfo: state.customerInfo, accountInfo: state.accountInfo);
      ...
      await _customerManagementService
          .updateCustomerInfo(CustomerManagementRequest(firstName: event.firstName, lastName: event.lastName));
      final AccountInfoResponse accountInfoResponse = await _accountService.fetchAccountDetails();
      yield CustomerDetailsState(
          type: CustomerDetailsStateType.fetched, customerInfo: state.customerInfo, accountInfo: accountInfoResponse);
    }
  }
}
